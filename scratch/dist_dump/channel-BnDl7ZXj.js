	...
	finalize: async (params) => {
		...
		if (finalCreds?.type === "secret") {
			if (await params.prompter.confirm({
				...
				try {
					const { loginMSTeamsDelegated } = await import("./oauth-BgGk7TEg.js");
					const { saveDelegatedTokens } = await import("./token---oXt4Ni.js");
					const { shouldUseManualOAuthFlow } = await import("./oauth.flow-DeI-UGpB.js");
...
const loadMSTeamsChannelRuntime = createLazyRuntimeNamedExport(() => import("./channel.runtime-CQEapBTL.js"), "msTeamsChannelRuntime");
...
const msteamsPlugin = createChatChannelPlugin({
	base: {
		...
		gateway: { startAccount: async (ctx) => {
			const { monitorMSTeamsProvider } = await import("./src-DGdS8kYd.js");
